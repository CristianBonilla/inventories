...
import { fetchUsers, fetchUsersFailure, fetchUsersSuccess, getUsers } from '@modules/auth/users/store/actions/users.actions';
  ...
  fetchUsers$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(_ => {
      const users$ = this.service.fetchUsers().pipe(
        delay(DEFAULT_WAIT),
        map(users => fetchUsersSuccess({ data: users })),
